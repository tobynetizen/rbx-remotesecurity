...
And finally, to finish things off, we can start a loop that calls our 'fake' remote event table which has no impact on the client-to-server bandwidth and blocks the exploiter from even seeing any traffic in their developer console using a remote spy.
